...
public class Calculator
{
    ...
    public int HighCard(PokerPlayer player)
    {
        ...
        return cards.Max(card => card.GetValue());
    }
    ...
    private int Pair(List<Card> cards)
    {
        ...
        return cards.GroupBy(card => card.GetValue()).Count(group => group.Count() == cardAmount) == 1 ? points : 0;
    }
    ...
    private int TwoPair(List<Card> cards)
    {
        const int points = 3;
        const int cardAmount = 2;
        return cards.GroupBy(card => card.GetValue()).Count(group => group.Count() == cardAmount) == 2 ? points : 0;
    }
    ...
    private int ThreeOfKind(List<Card> cards)
    {
        ...
        return cards.GroupBy(card => card.GetValue()).Any(group => group.Count() == cardAmount) ? points : 0;
    }
    ...
    private int Flush(List<Card> cards)
    {
        const int points = 6;
        const int cardAmount = 5;
        return cards.GroupBy(card => card.GetSuit()).Count(group => group.Count() >= cardAmount) == 1 ? points : 0;
    }
    ...
    private int RoyalFlush(List<Card> cards)
    {
        ...
        const int cardAmount = 5;
        const int aceValue = (int)Rank.Ace;
        var combs = Combinations(cards, cardAmount);
        if (combs.Any(set => FiveInRow(set.ToList()) && Flush(set.ToList())==pointsFlush && cards.Max(card => card.GetValue())==aceValue))
        {
            return points;
        }
        return 0;
    ...
    private bool FiveInRow(List<Card> cards)
    {
        ...
        return cards.GroupBy(card => card.GetValue()).Count() == cards.Count() &&
               cards.Max(card => card.GetValue()) - cards.Min(card => card.GetValue()) == valueDiff;
    }
